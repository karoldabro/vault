...
| [[business-persona-family]] | Business packs (sales·seo·support·business·startup-eval) are opt-in critique lenses grounded in installed skills/MCPs; numbers route to the shared `business/data-evidence` critic; multi-pack seating via `personas.use` list, one architect seat, §2.2 selection | `personas/{sales,seo,support,business,startup-eval,marketing}.md`, `personas/_shared/business/**`, `personas/_resolution.md` |
...
| [[requirements-spec-vs-established]] | Plan-time business logic is a `requirements/` SPEC (aspirational, REQ-NN + axis tags, 1+ repos); `features/` is the established form promoted at `/v-capture` §5b carrying REQ ids; id chain grounds tests; never write aspirational rules into `features/` | `commands/v-pm/steps/**`, `commands/v-capture.md`, `commands/v-team/steps/{00,03,04}`, `templates/_features/requirements.md` |
...
| [[propose-front-gates]] | PROPOSE §3a opens with clarify (assumptions + AskUserQuestion for plan-changing doubts; **hard-blocks / always waits** on a no-safe-default fork) then external research (ground vs the wild; reconcile contradicting consensus in writing); understand + ground before planning | `commands/v-work/steps/03-propose.md`, `commands/v-team/steps/03-propose-loop.md`, `commands/v-work/steps/01-analyze.md`, `tool-playbook.md` |
| [[cross-project-conversation-workspace]] | Coordinate multi-project sessions via a shared file-based conversation (state-in-filename, derived ledger not written, pull pickup + a push `status` surface, honest next-open latency, deterministic contract-drift not LLM prose) | `commands/v-pm.md`, `commands/v-pm/steps/**`, `commands/v-team/steps/00-feature-pickup.md`, `templates/_features/**`, `vault-guide.md` §13 |
| [[user-facing-communication]] | Every user-facing line is governed by the one shared module `_shared/communication.md` (bind, never duplicate); output is two-layer (decision to the terminal, design to the artifact) and the split must DELETE fields; `Impact` mandatory at every gate; omit-when-empty cuts green never amber; subagent text needs its own cap because output styles don't reach subagents | `commands/_shared/communication.md`, `output-styles/director.md`, `commands/v-*.md`, all `## Required output` step files, `install.sh` |
