...
```bash
$ HOME= bash -c 'set -euo pipefail; echo "[${HOME}/.openviking]"'
[/.openviking]
```

This is easy to fix once and easy to miss everywhere else. During the OpenViking removal
([[../decisions/ADR-019-drop-openviking-dependency]]) the guard was added to the new remover, and the
identical bug stayed live in the sibling script for a whole commit — `vault-uninstall.sh` still did a
raw `rm -rf "${VAULT_HOME}/_global"`, which with an empty `HOME` resolved to `/vault/_global`. A
review caught it; `set -euo pipefail` and code review of the new file did not.

## Examples
- Do: `safe_rm_under_home "${HOME}/.openviking"` — refuses empty/`/` `HOME`, refuses paths outside
...
## Applies-to
`bin/*.sh` (esp. `vault-uninstall.sh`, `remove-openviking.sh`), `lib/installers.sh`, any future
