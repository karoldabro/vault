...
| D-006 | A session took work it did not write into its own commit: first with a directory-wide `git add`, then through `git commit -am`, which the first repair did not cover | `scripts/staging-hook.sh` denies `git add -A`, `git add .`, `git commit -a`, `git commit -am`, a pathspec-less `git commit`, `git commit --amend` and `git reset --hard` before the call runs, and is registered as a `PreToolUse` entry; the prose prohibition it replaces did not hold, and the first hook covered one verb of the action it names | `tests/unit/staging-hook.bats:1`, `tests/unit/staging-hook.bats:70` | 1 |
| D-007 | Two plans in one vault could name the same check script, so each would grade itself against the other's check | `bin/gate.sh criteria` refuses a check path another plan in the same directory claims | `tests/unit/gate.bats:519` | 0 |
| D-008 | A contract document named checks that were never built, so a session read its work as gated when nothing ran it | the document lists only what `bin/gate.sh` answers to; the unbuilt checks move to `vault/architecture/session-gates-unbuilt.md`; three scripts diff the documents against the real dispatcher | `checks/doc-truth-SC-1.sh` | 0 |
