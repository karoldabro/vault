...
# ADR-027 — a campaign command that refuses without a stack the operator agreed to lose
...
1. **`/v-loop` refuses without a disposable stack the operator names in words.** Not a running
   system, which every finished `/v-work` leaves behind. The precondition is a stack someone agreed
   to lose, and it is what keeps the campaign's destructive commands away from a working database.
2. **`/v-loop` sits outside the `/v-do` → `/v-work` → `/v-team` ladder.** That ladder builds; this
   verifies and repairs what is already built, so neither escalates into the other. The command
   names the cheaper alternative for four situations.
...
4. **The agent that writes a fix does not produce its retest verdict.** An agent grading its own
   work skews positive.
5. **Concurrency is expressed as disjoint conflict sets, not as a number.** "Three agents at a time"
   is a count no session keeps; "one tester unless the conflict sets are disjoint" is decidable from
   the ledger row being written. The two campaigns disagree on the number anyway.
6. **Every campaign rule names the sanctioned action rather than the forbidden one, and
   `checks/v-loop-SC-4.sh` holds both files to requirements outnumbering prohibitions.** The reason
...
- **Adapter-local, not general:** the conflict-scope taxonomy and the browser rules come from the web
  campaign alone, and the device diagnostics from the mobile one. A third campaign on another stack
  either confirms them or deletes them.
...
- **Watch:** whether `/v-loop` is invoked without a disposable stack being named. A session that
  starts a campaign against the operator's working stack means the precondition failed the way
  `/v-team`'s did, and the answer is a check rather than another sentence.
