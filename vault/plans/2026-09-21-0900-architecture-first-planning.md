...
| S9 | stack packs for Laravel, Nuxt, Flutter, Python and SQL, validated on `recycling-api` and one Nuxt repo; the plan is `vault/plans/2026-09-22-0900-stack-packs.md` | /v-work | done | S4, S5 | 2026-09-22 | `bin/gate.sh verdict vault/plans/2026-09-22-0900-stack-packs.md --run` reports SC-1 to SC-7 MET |
...
| S11 | spec-reading SQL probes `spec-tables` and `spec-naming`, and the `data-model` and `naming` auditors of `commands/_shared/plan-probes.md` | /v-team | done | S5 | 2026-09-22 | `bin/gate.sh verdict vault/plans/2026-09-22-1023-spec-reading-probes.md --run` reports SC-1 to SC-8 MET; `./tests/run.sh tests/unit/probe.bats tests/unit/plan-probes.bats` 55 of 55; two real BLOCKER bugs found and fixed in EXECUTE diff review, both re-verified live |
| S12 | `/v-pm` side of the contracts table: `templates/_features/project-shard.md` gains `depends` and `## Cross-session contracts`, `commands/v-pm/steps/04-seed-workspace.md` seeds them, `commands/v-pm/steps/07-status.md` runs `bin/gate.sh master` on each shard, `tests/unit/v-pm.bats` guards it, and the Sessions close duty of `commands/v-work/steps/05-commit-capture.md` covers ordinary master plans | /v-team | done | S6 | 2026-09-21 | `bin/gate.sh verdict vault/plans/2026-09-21-2040-pm-shard-contracts.md --run` reports SC-1 to SC-4 MET |
