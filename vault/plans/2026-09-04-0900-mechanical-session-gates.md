...
| O-8 | `scripts/completion-hook.sh` BLOCKS a turn end. The two rules `scripts/doc-lint-hook.sh` merely reports on score 100% and 94.4%, so reporting may achieve the same at less friction. The two cases differ — doc-lint reports a mechanical fact at write time, while this one targets a claim the model cannot self-assess — so neither result transfers. `vault/check-budget.md` records fires; a block that was never needed is what would settle it | OPEN |
| O-9 | "Leave pushing to the operator" cannot be measured as written: a push the operator asked for and one taken unprompted leave the same trace. It is listed as prose in `vault/check-budget.md` rather than left looking enforceable | ACCEPTED |
| O-10 | The 50-character commit-subject clause is deleted, not given a counter. It scored 18.1%, it is cosmetic, and a blocked commit over a 52-character subject is the friction that gets `GATE=off` set permanently | ACCEPTED |
| O-11 | "Enforced rules average 98.1%" cannot support the gates: all three were enforced from their first day, so the study cannot separate enforcement working from tractable rules getting enforcement early. The gates rest on the false-success measurement instead, which is independent | ACCEPTED |
...
| SC-2 | WHEN a criterion names a check THE SYSTEM SHALL require a committed script and refuse a command written inline in the plan | unit | command | `checks/SC-2.sh` | exit 0 | MET | `checks/SC-2.sh` exited 0 · gate.bats: 49 cases, 0 failing |
| SC-3 | WHEN a plan carries no criterion that runs the real system THE SYSTEM SHALL refuse it | unit | command | `checks/SC-3.sh` | exit 0 | MET | `checks/SC-3.sh` exited 0 · gate.bats: 49 cases, 0 failing |
...
| SC-7 | WHEN a repo declares no test, lint and delivery commands THE SYSTEM SHALL refuse at the first step of a session | unit | command | `checks/SC-7.sh` | exit 0 | MET | `checks/SC-7.sh` exited 0 · gate.bats green and config exists |
...
| C-01 | `bin/gate.sh` | modify | Edit | `criteria` refuses a plan with no row of `kind: delivery`. A delivery row runs the real system and asserts this change appears in what the run produced. A repo with no runtime declares `no-runtime:` in frontmatter | SC-3 | same | DONE |
...
| F-01 | `vault/decisions/ADR-026-mechanical-session-gates.md` | create | Write | the eight decisions above with the evidence each rests on | SC-6 | `bin/doc-lint.sh` on the file | DONE |
| F-02 | `install.sh`, `INSTALL.md` | modify | Edit | ship `bin/gate.sh`, `scripts/completion-hook.sh`, `bin/rule-count.sh` and the hook registrations | SC-7 | `tests/unit/install.bats` | DONE |
| F-03 | `vault-guide.md`, `vault/_moc.md`, `vault/decisions/_inventory.md` | modify | Edit | one section naming the gate and the delivery check; index ADR-026 and the two new vault surfaces | SC-6 | `bin/doc-lint.sh --changed` | DONE |
