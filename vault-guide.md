...
through a `features/<feature>` symlink. Protocol: [docs/cross-project-workspaces.md](docs/cross-project-workspaces.md).
...
├── reports/                 # /v-report problems found during other work, to fix or drop later
...
| `handoff.md` | What the next session carries on. Written by `/v-handoff`; lives in `handoffs/`. |
| `report.md` | A problem found during other work. Written by `/v-report`; lives in `reports/`. |
...
| What the next session must carry on: left to do, do not touch, unverified, next command | `handoffs/` | `YYYY-MM-DD-HHMM-<slug>.md` |
| A problem found while doing something else, to fix or drop in a later session | `reports/` | `YYYY-MM-DD-HHMM-<slug>.md` |

**Where a problem goes.** A problem inside the current plan's scope stays in that plan's `## Open &
deferred` section. `reports/` holds only a problem outside the scope of the work that found it, and
`vault/defect-ledger.md` holds a defect this repo has already repaired, with whether it came back.

**Handoff or session:** a handoff states what is still true and is read before the next session
starts; a session capture states what happened and is read when someone asks why. A session that
finished its work writes only the capture.
...
in the plugin's own `.claude-plugin/plugin.json`. Install with `/v-plugin <repo>` — `owner/repo`, a URL, a path, or a name this
marketplace lists; it clones, shows what it will trust, and asks (`list`, `doctor`, `remove`). Nothing is scanned for: you name every path, and registering a plugin
...
`/v-pm` plans a feature once, project-agnostically, into `~/vault/_features/`, and each project's
`/v-team <feature>` session coordinates through files in it. The home, layout, requirements
lifecycle, sizing, derived status, conversation protocol, auto-pickup and latency contract are in
[docs/cross-project-workspaces.md](docs/cross-project-workspaces.md).
