...
Each file is a Claude Code slash-command definition, and its `description:` frontmatter is what users see
in the command help.
...
Each command resolves `$VAULT_FRAMEWORK_PATH` in its own header, preferring `${CLAUDE_PLUGIN_ROOT}` when
Claude Code substituted one. That is what lets the same file work from a plugin cache directory and from
a git clone. `../vault-guide.md` §1.1 holds the resolution order.
...
`/v-cr` points the same panel at a remote PR or MR instead of authoring code, reusing
`_shared/critic-panel.md`, the persona resolution, and the adapters under `v-cr/adapters/` and
`v-cr/tasks/`. Pure parsing logic lives in `../lib/forge-detect.sh` and `../lib/cr-helpers.sh`, both
unit-tested.

`/v-ask` and `/v-do` are the **light siblings**: single-file, no step subdirectory, no approval gate.
Both escalate to `/v-work` or `/v-team` the moment scope grows.
...
- **`communication.md`** — how every command writes **to the user**. Bound at the top of each `v-*.md`
  and each step file owning a `## Required output` block. It governs user-facing prose only; forge
  comments defer to `/v-cr`'s own brevity rule, which serves a different reader.
...
- **`vault-sync.md`** — how commands pull and push an out-of-repo vault through `../bin/vault-sync.sh`.

`../output-styles/director.md` applies the same writing rules to sessions outside a v-* command; the
plugin loads it directly, `install.sh` links it into `~/.claude/output-styles/`, and either way it is
opt-in through `/config`. `../vault-guide.md` §11 holds the full command reference.

## Why symlinks instead of copies

This applies to the `install.sh` route only. Pulling the framework repo updates the symlinked commands at
once, with no per-machine reinstall. Edit `install.sh` if you would rather have copies.
