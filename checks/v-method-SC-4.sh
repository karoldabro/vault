#!/usr/bin/env bash
# SC-4 — the two v-method files are written in requirement form, not prohibition form.
#
# Prohibition-shaped rules fall from 73% compliance at turn 5 to 33% by turn 16; requirements hold.
# `/v-method` runs an intake, a routing pass and a write, so it has turns to lose across. Fails when
# prohibitions outnumber requirements in either file. Same two patterns bin/rule-count.sh uses, so
# one grammar governs the whole framework.
set -uo pipefail
