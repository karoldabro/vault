#!/usr/bin/env bash
# SC-8 — the touched test files pass, and the full unit suite fails only the five tests that fail on HEAD.
...
[ -f "$root/tests/unit/rule.bats" ] || { echo "tests/unit/rule.bats not written yet — cannot say"; exit 2; }
...
for f in rule probe probe-panel v-team v-cr cr-rule-routing; do
