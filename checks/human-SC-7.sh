...
awk '{ print } /^```mermaid/ && !done { print "click A href \"javascript:alert(1)\""; print "%%{init: {\"securityLevel\": \"loose\"}}%%"; print "A --> B %% see javascript:x"; done = 1 }' "$fx/arch/code-complete.arch.md" > "$tmp/arch/code-complete.arch.md"
...
grep -qF 'orders ||--o{ order_lines : has' "$page" || fail "the legitimate diagram lines were lost with the hostile ones"
exit 0
