#!/usr/bin/env bash
# SC-4 — registration refuses a directory that is not a plugin, and writes nothing
#
...
cases=(
    "add on a directory with no extend/plugin.tsv writes no registry row"
    "add on a plugin declaring a point whose file is absent writes no registry row"
    "add on a plugin whose point file is not executable writes no registry row"
    "add on a path traversing a symlink writes no registry row"
    "no code path other than vault-plugin.sh writes the registry"
)
