...
#
# Fails when the command file would let a session begin against the operator's own stack, start with
# the feature unnamed, resume with its round counter reset, or run without a ceiling.
set -uo pipefail
...
# precondition: a stack the operator has named as disposable, not merely a running one
need 'disposable' 'the precondition does not require a disposable stack'
need 'names? (it|the stack)|operator names' 'the operator is never asked to name the stack'

# intake: the five decisions taken in the first exchange
need 'feature' 'intake does not ask which feature'
need 'branch' 'intake does not ask for the branch'
...
need 'TESTER-BRIEF' 'the envelope does not carry the tester brief'
...
[ "$fail" -eq 0 ] && printf '  OK  v-loop.md carries the precondition, the intake, both caps, resume and the envelope\n'
exit "$fail"
