#!/usr/bin/env bash
# SC-2 — a failing plugin entry point warns once and never aborts its host
#
...
cases=(
    "a point exiting 1 warns once and leaves vault-init at exit 0"
    "a sourced point cannot abort the host through set -e"
    "a registry row whose path no longer exists warns once and keeps the row"
)
